...
namespace KHCN.Api.Controllers.KHCN
{
    ...
    public class HoSoDieuChinhKhacController : BaseController<HoSoDieuChinhKhacController>
    {
        ...
        public HoSoDieuChinhKhacController(IHoSoDieuChinhKhacRepository hosodieuchinhkhacRepository, ITaiLieuDinhKemRepository taiLieuDinhKemRepository, IUnitOfWork unitOfWork, FileService fileService, IConfiguration configuration, IPathProvider pathProvider, IUserProvider userProvider)
        {
            _hosodieuchinhkhacRepository = hosodieuchinhkhacRepository;
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<KHCN_HoSoDieuChinhKhac>>> GetAll()
        {
            try
            {
                sw.Restart();
                var res = await _hosodieuchinhkhacRepository.GetAllAsyn();
                sw.Stop();
                log.LogInformation($"[{HttpContext.Connection.RemoteIpAddress.ToString()}]-[KHCN_HoSoDieuChinhKhac.GetAll]. Get data success. Time process: {sw.Elapsed.Seconds} seconds.");
            ...
            {
                sw.Stop();
                log.LogError($"[{HttpContext.Connection.RemoteIpAddress.ToString()}]-[KHCN_HoSoDieuChinhKhac.GetAll]. Get data error. Detail: {ex}");
                return BadRequest();
            }
        }

        [HttpGet]
        public async Task<ActionResult<PagedResults<KHCN_HoSoDieuChinhKhac>>> GetAllPaging(int? page, int? pageSize, string orderBy = nameof(KHCN_HoSoDieuChinhKhac.Id), bool ascending = true)
        {
            try
            {
                sw.Restart();
                var res = await CreatePagedResults(_hosodieuchinhkhacRepository.GetAll(), page.Value, pageSize.Value, orderBy, ascending);
                sw.Stop();
                log.LogInformation($"[{HttpContext.Connection.RemoteIpAddress.ToString()}]-[KHCN_HoSoDieuChinhKhac.GetAllPaging]. Get data success. Time process: {sw.Elapsed.Seconds} seconds.");
            ...
            {
                sw.Stop();
                log.LogError($"[{HttpContext.Connection.RemoteIpAddress.ToString()}]-[KHCN_HoSoDieuChinhKhac.GetAllPaging].  Get data page {page} pagesize {pageSize} error. Detail: {ex}");
                return BadRequest();
            }
        }

        [HttpGet("{id}")]
        public async Task<ActionResult<KHCN_HoSoDieuChinhKhac>> GetById(int id)
        {
            try
            {
                sw.Restart();
                var obj = await _hosodieuchinhkhacRepository.GetAsync(id);

                if (obj == null)
                {
                    sw.Stop();
                    log.LogError($"[{HttpContext.Connection.RemoteIpAddress.ToString()}]-[KHCN_HoSoDieuChinhKhac.GetById]. Get by id = {id} error. Cannot find obj by id ={id}");
                    return NotFound();
                }

                sw.Stop();
                log.LogInformation($"[{HttpContext.Connection.RemoteIpAddress.ToString()}]-[KHCN_HoSoDieuChinhKhac.GetById]. Get by id = {id} success. Time process: {sw.Elapsed.Seconds} seconds.");
                return Ok(obj);
            }
            catch (Exception ex)
            {
                sw.Stop();
                log.LogError($"[{HttpContext.Connection.RemoteIpAddress.ToString()}]-[KHCN_HoSoDieuChinhKhac.GetById]. Get by id = {id} error. Detail: {ex}");
                return BadRequest();
            }
        }

        [HttpPut("{id}")]
        public ActionResult Put(int id, [FromForm]KHCN_HoSoDieuChinhKhac_DTO obj, List<IFormFile> lstFile)
        {
            if (obj == null)
            {
                log.LogError($"[{HttpContext.Connection.RemoteIpAddress.ToString()}]-[KHCN_HoSoDieuChinhKhac.Put]. Put by id = {id} error. Detail: Model put is null");
                return NotFound();
            }
            ...
            try
            {
                ...
                {
                    sw.Restart();
                    var currentObj = _hosodieuchinhkhacRepository.Get(id);
                    if (currentObj == null)
                    {
                        sw.Stop();
                        log.LogError($"[{HttpContext.Connection.RemoteIpAddress.ToString()}]-[KHCN_HoSoDieuChinhKhac.Put]. Put by id = {id} error. Detail: Cannot find object by id = {{id}}");
                        return BadRequest();
                    }

                    var model = new KHCN_HoSoDieuChinhKhac()
                    {
                        Id = obj.Id.Value,
                    ...
                    _unitOfWork.Context.Set<KHCN_HoSoDieuChinhKhac>().Update(model);
                    ...
                    sw.Stop();
                    log.LogInformation($"[{HttpContext.Connection.RemoteIpAddress.ToString()}]-[KHCN_HoSoDieuChinhKhac.Put]. Put by id = {id} success. Time process: {sw.Elapsed.Seconds} seconds.");
                    return Ok(obj);
                }
                else
                {
                    var errors = string.Join(",", ModelState.SelectMany(m => m.Value.Errors).Select(e => e.ErrorMessage).ToList());
                    log.LogError($"[{HttpContext.Connection.RemoteIpAddress.ToString()}]-[KHCN_HoSoDieuChinhKhac.Put]. ModelState invalid. Detail: {errors}");
                    return BadRequest();
                }
            }
            ...
            {
                sw.Stop();
                log.LogError($"[{HttpContext.Connection.RemoteIpAddress.ToString()}]-[KHCN_HoSoDieuChinhKhac.Put]. Put by id = {id} error. Detail: {ex}");
                return BadRequest();
            }
        }

        [HttpPost]
        public ActionResult<KHCN_HoSoDieuChinhKhac> Post([FromForm]KHCN_HoSoDieuChinhKhac_DTO obj, List<IFormFile> lstFile)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    _unitOfWork.Context.Set<KHCN_HoSoDieuChinhKhac>().Add(model);
                    _unitOfWork.Context.Set<KHCN_TaiLieuDinhKem>().AddRange(lstInfoAttachment);
                    _unitOfWork.Commit();

                    sw.Stop();
                    log.LogInformation($"[{HttpContext.Connection.RemoteIpAddress.ToString()}]-[KHCN_HoSoDieuChinhKhac.Post]. Post by masonv = {obj.MaSoNhiemVu} success. Time process: {sw.Elapsed.Seconds} seconds.");
                ...
                {
                    var errors = string.Join(",", ModelState.SelectMany(m => m.Value.Errors).Select(e => e.ErrorMessage).ToList());
                    log.LogError($"[{HttpContext.Connection.RemoteIpAddress.ToString()}]-[KHCN_HoSoDieuChinhKhac.Post]. ModelState invalid. Detail: {errors}");
            ...
            {
                sw.Stop();
                log.LogError($"[{HttpContext.Connection.RemoteIpAddress.ToString()}]-[KHCN_HoSoDieuChinhKhac.Post]. Post by masonv = {obj.MaSoNhiemVu} error. Detail: {ex}");
                return BadRequest();
            }
        }

        [HttpDelete("{id}")]
        public async Task<ActionResult<KHCN_HoSoDieuChinhKhac>> Delete(int id)
        {
            try
            {
                sw.Restart();
                ...
                {
                    sw.Stop();
                    log.LogError($"[{HttpContext.Connection.RemoteIpAddress.ToString()}]-[KHCN_HoSoDieuChinhKhac.Delete]. Delete by id = {id} error. Detail: Cannot find obj by id ={id}");
                    return NotFound();
                }

                await _hosodieuchinhkhacRepository.DeleteAsync(obj);
                sw.Stop();
                log.LogInformation($"[{HttpContext.Connection.RemoteIpAddress.ToString()}]-[KHCN_HoSoDieuChinhKhac.Delete]. Delete by id = {id} success. Time process: {sw.Elapsed.Seconds} seconds.");
                return Ok();
            }
            catch (Exception ex)
            {
                sw.Stop();
                log.LogError($"[{HttpContext.Connection.RemoteIpAddress.ToString()}]-[KHCN_HoSoDieuChinhKhac.Delete]. Delete by id = {id} error. Detail: {ex}");
                return BadRequest();
            }
        }
        ...
        {
            return _hosodieuchinhkhacRepository.Get(id.Value) == null;
        }
    }
}